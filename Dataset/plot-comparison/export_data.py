...
root_path = r"C:\Users\Andrew\Documents\Project\Time Series\TSDatasets/"
    ...
    for filename in listdir:
        ...
        try:
            ...
            try:
                ...
                pass
    return list_mse, list_mae, list_params_value, list_order
...
def get_index_from_dict(method_error, list_index):
    idx = []
    for id, val in enumerate(method_error[3]):
...
def plot_data_errors_on_a_dataset(num_type_display, dataset_order, dict_method_error):
    ...
    for name_method in dict_method_error:
        print(name_method)
        # dict_method_error[name_method] belongs to [mse, mae, param, list_time]
        data_plot_mse = dict_method_error[name_method][0]
        data_plot_mae = dict_method_error[name_method][1]
        data_plot_param = dict_method_error[name_method][2]
        try:
            # keep the list of hours like 1,2,3,..24, 32,48
            hours_order = dict_method_error[name_method][3]
        except Exception:
        ...
        num_par = [data_plot_param[index] for index in list_hour]

        # [Name, Model-size, Time-step, MSE]
        for i, mse, mae, nb_param in zip(id_index, error_mse, error_mae, num_par):
            data.append([name_method, int(nb_param), int(i), mse])
    return data, dataset_order
...
def compare_auto_correlation_Household():
    ...
    # TCN auto-generated search
    path_folder2 = f"{root_path}automl_searching/{list_dataset[num_data]}_result_auto/*.txt"
    dict_method_error["auto-tcn"] = get_error(path_folder2)

    # LSTM
    path_folder2 = f"{root_path}automl_searching/{list_dataset[num_data]}_result_lstm/*.txt"
    dict_method_error["lstm"] = get_error(path_folder2)

    # GRU
    path_folder2 = f"{root_path}automl_searching/{list_dataset[num_data]}_result_gru/*.txt"
    dict_method_error["gru"] = get_error(path_folder2)

    # Autocorrelation-Dilated TCN
    path_folder2 = f"{root_path}auto_correlation/{list_dataset[num_data]}/{list_dataset[num_data]}_result/*.txt"
    dict_method_error["Autocorrelation-Dilated TCN"] = get_error(path_folder2)
    ...
    for fn in [2, 3, 4]:
        # AUTO-CORRELATION
        correlation_auto_pth = f"{root_path}auto_correlation/{list_dataset[num_data]}/{list_dataset[num_data]}_auto_fix_{fn}layers/*.txt"
        dict_method_error[f"auto-stride-{fn}layers"] = get_error(correlation_auto_pth)

    return plot_data_errors_on_a_dataset(0, num_data, dict_method_error)
...
def compare_auto_correlation_CNU():
    ...
    # TCN auto-generated search
    path_folder2 = f"{root_path}automl_searching/{list_dataset[num_data]}_result_auto/*.txt"
    dict_method_error["auto-tcn"] = get_error(path_folder2)  # [mse, mae, param, list_time]

    # Autocorrelation-Dilated TCN
    path_folder2 = f"{root_path}auto_correlation/{list_dataset[num_data]}/{list_dataset[num_data]}_result/*.txt"
    dict_method_error["Heuristic-stride-TCN"] = get_error(path_folder2)

    for fn in [2, 3, 4]:
        # AUTO-CORRELATION
        correlation_auto_pth = f"{root_path}auto_correlation/{list_dataset[num_data]}/{list_dataset[num_data]}_auto_fix_{fn}layers/*.txt"
        dict_method_error[f"Stride-TCN-{fn}layers"] = get_error(correlation_auto_pth)
